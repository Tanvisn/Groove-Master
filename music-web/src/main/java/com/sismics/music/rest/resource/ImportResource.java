...
public class ImportResource extends BaseResource {
    ...
    public Response list() {
        ...
        List<ImportAudioFile> importedFileList = AppContext
                .getInstance()
                .getImportAudioService()
                .getImportedFileList();
    
        Collections.sort(importedFileList, (file1, file2) -> Long.compare(file1.getFile().lastModified(), file2.getFile().lastModified()));
    
        JsonArrayBuilder items = Json.createArrayBuilder();
        for (ImportAudioFile importedFile : importedFileList) {
            JsonObjectBuilder item = createItem(importedFile);
            items.add(item);
        }
    ...
    private JsonObjectBuilder createItem(ImportAudioFile importedFile) {
        JsonObjectBuilder item = Json.createObjectBuilder()
                .add("file", importedFile.getFile().getName());
        addJsonProperty(item, "title", importedFile.getTitle());
        addJsonProperty(item, "album", importedFile.getAlbum());
        addJsonProperty(item, "artist", importedFile.getArtist());
        addJsonProperty(item, "albumArtist", importedFile.getAlbumArtist());
        addJsonProperty(item, "order", importedFile.getOrder());
        addJsonProperty(item, "year", importedFile.getYear());
        return item;
    }
    
    private void addJsonProperty(JsonObjectBuilder object, String name, Object value) {
        if (value != null) {
