...
public class CollectionWatchService extends AbstractExecutionThreadService {
    ...
    protected void run() throws Exception {
        while (isRunning()) {
            ...
            for (WatchEvent<?> event : watchKey.pollEvents()) {
                Path path = getPathFromEvent(event, dir);
                Directory directory = getParentDirectory(path);
    
                if (event.kind() == StandardWatchEventKinds.ENTRY_CREATE) {
                    handleCreateEvent(directory, path);
                } else if (event.kind() == StandardWatchEventKinds.ENTRY_DELETE) {
                    handleDeleteEvent(directory, path);
                }
            }
    ...
    private Path getPathFromEvent(WatchEvent<?> event, Path dir) {
        @SuppressWarnings("unchecked")
        WatchEvent<Path> eventPath = (WatchEvent<Path>) event;
        return dir.resolve(eventPath.context());
    }
    
    private void handleCreateEvent(Directory directory, Path path) throws IOException {
        if (Files.isDirectory(path, LinkOption.NOFOLLOW_LINKS)) {
            handleNewDirectory(directory, path);
        } else {
            indexNewFile(directory, path);
        }
    }
    
    private void handleNewDirectory(Directory directory, Path path) throws IOException {
    ...
    private void handleDeleteEvent(Directory directory, Path path) {
